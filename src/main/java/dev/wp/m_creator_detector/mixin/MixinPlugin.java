package dev.wp.m_creator_detector.mixin;

import dev.wp.m_creator_detector.MCreatorDetector;
...
public class MixinPlugin implements IMixinConfigPlugin {
    public static final Logger LOGGER = LogManager.getLogger(MCreatorDetector.MOD_ID);
    ...
    public MixinPlugin() {
        ...
        if (!MODS_FAILED_TO_DETECT.isEmpty()) {
            if (MODS_FAILED_TO_DETECT.size() == modFileInfoList.size()) {
                LOGGER.fatal("MCreatorDetector failed to detect files because your mods folder are locked, try to restart your computer!");
                return;
            }
